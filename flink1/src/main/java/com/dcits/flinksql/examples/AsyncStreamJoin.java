...
public class AsyncStreamJoin {
    public static void main(String[] args) throws Exception {
        ...
        DataStream<String> _inputb =
        		AsyncDataStream.unorderedWait(inputb, new AsyncDatabaseRequest(), 30000, TimeUnit.MILLISECONDS, 100);
        DataStream<String> _inputa =
        		AsyncDataStream.unorderedWait(inputa, new AsyncDatabaseRequest(), 30000, TimeUnit.MILLISECONDS, 100);
        ...
        DataStream<Row> _tableb = _inputb.map(x->toRow(x)).returns(new RowTypeInfo(fieldTypes));
        DataStream<Row> _tablea = _inputb.map(x->toRow(x)).returns(new RowTypeInfo(fieldTypes));
